...
export const createDelivery = onRequest(async (req, res) => {
    try {
        ...
        await getUser(req);
        const { customer_uid, pickup, dropoff, packageDetails, vehicleType } = req.body;
...
export const assignDelivery = onRequest(async (req, res) => {
    try {
        ...
        await getUser(req);
        const { deliveryId, driver_uid } = req.body;
 ...
 */
export const checkGeofenceAndAlerts = onValueUpdated(`deliveryLocations/{deliveryId}`, async (event) => {
    ...
    try {
        ...
        for (const doc of geofencesSnapshot.docs) {
            ...
            const distance = getDistance({ latitude: newLocation.lat, longitude: newLocation.lng }, { latitude: geofenceLat, longitude: geofenceLng });
            ...
            if (distance <= radius) {
                ...
                console.log(`Alert: Driver for delivery ${deliveryId} entered geofence ${geofence.name}.`);
            }
        }
    }
    catch (error) {
