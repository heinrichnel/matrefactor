...
import { Input, Select, TextArea, FileUpload } from '../ui/FormElements'; // Ensure FileUpload is imported if used
import Modal from '../ui/Modal';
import { User as UserRound, AlertTriangle, CheckCircle, Filter, Plus, FileUp, X, Save, Eye, Edit, Trash2, Shield, Clock, MapPin } from 'lucide-react';
...
const DriverPerformanceOverview: React.FC = () => {
  ...
  const filteredEvents = useMemo(() => {
    return driverBehaviorEvents.filter(event => {
      ...
      const matchesDateRange = (!dateRange.start || event.eventDate >= dateRange.start) && (!dateRange.end || event.eventDate <= dateRange.end);
      return matchesDriver && matchesEventType && matchesSeverity && matchesStatus && matchesDateRange;
    });
  ...
  const summary = useMemo(() => {
    ...
    const unresolvedEvents = filteredEvents.filter(e => (e.status || 'pending') !== 'resolved').length;
    ...
    return { totalEvents, criticalEvents, highSeverityEvents, unresolvedEvents, topEventTypes, highRiskDrivers };
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <div className="flex space-x-2">
              <Button size="sm" variant="outline" onClick={clearFilters} icon={<X className="w-4 h-4" />}>
                Clear Filters
              </Button>
              <Button
                size="sm"
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <Select
              ...
              options={[{ value: '', label: 'All Drivers' }, ...DRIVERS.map(driver => ({ value: driver, label: driver }))]}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Critical Events</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {summary.highRiskDrivers.map((driver, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{driver.eventCount}</td>
                        ...
                        <div className="flex items-center">
                          <div className="w-16 bg-gray-200 rounded-full h-2.5 mr-2">
                            <div className={`h-2.5 rounded-full ${
                              driver.score > 80 ? 'bg-green-500' :
                              driver.score > 60 ? 'bg-yellow-500' : 'bg-red-500'
                            }`} style={{ width: `${driver.score}%` }}></div>
                          </div>
        ...
        <CardContent>
          {filteredEvents.length > 0 ? (
                  ...
                  {filteredEvents.map((event) => (
                    <tr key={event.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSeverityClass(event.severity)}`}>
                          {event.severity}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="text-center py-8">
              ...
              <p className="mt-1 text-sm text-gray-500">No driver behavior events match your current filters.</p>
              <div className="mt-6">
                <Button
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Select
              ...
              options={[{ value: '', label: 'Select Driver' }, ...DRIVERS.map(driver => ({ value: driver, label: driver }))]}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFormChange('driverName', e.target.value)}
            ...
            <Select
              ...
              options={[{ value: '', label: 'Select Fleet Number' }, ...FLEET_NUMBERS.map(fleet => ({ value: fleet, label: fleet }))]}
              onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFormChange('fleetNumber', e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => handleFormChange('eventDate', e.target.value)}
            ...
            <Input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => handleFormChange('eventTime', e.target.value)}
          ...
          <Select
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFormChange('eventType', e.target.value)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFormChange('severity', e.target.value as any)}
            ...
            <Select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFormChange('status', e.target.value as any)}
            />
          </div>
          ...
          <Input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleFormChange('location', e.target.value)}
          ...
          <TextArea
            ...
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) => handleFormChange('actionTaken', e.target.value)}
          ...
          <Input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleFormChange('points', parseInt(e.target.value) || 0)}
            placeholder="Penalty points"
          />
          <p className="mt-1 text-xs text-gray-500">Points are automatically assigned based on event type but can be adjusted.</p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSeverityClass(selectedEvent.severity)}`}>
              ...
              <div>
                ...
                <p className="mt-1">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClass(selectedEvent.status)}`}>
              ...
              <div className="col-span-2">
                ...
                <p className="mt-1 text-sm text-gray-900">{selectedEvent.location || 'Not specified'}</p>
              </div>
              <div className="col-span-2">
