        ...
        <div className="p-6">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between px-2 py-4 overflow-x-auto">
                  <div className="flex flex-col items-center">
                    <div
                      className={`rounded-full p-2 ${inspection.status.inspection === "COMPLETED" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                    >
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={`rounded-full p-2 ${inspection.status.workOrder ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"}`}
                    >
                      <FileText className="h-6 w-6" />
                    </div>
                    <span className="text-sm mt-1">Work Order</span>
                    <span className="text-xs text-gray-500">
                      {inspection.status.workOrder || "PENDING"}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={`rounded-full p-2 ${inspection.correctiveAction === "TAKEN" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                    >
                      <Wrench className="h-6 w-6" />
                    </div>
                    <span className="text-sm mt-1">Repair</span>
                    <span className="text-xs text-gray-500">
                      {inspection.correctiveAction === "TAKEN" ? "COMPLETED" : "PENDING"}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={`rounded-full p-2 ${inspection.status.completion ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                    >
                      <CheckCircle className="h-6 w-6" />
                    </div>
                    <span className="text-sm mt-1">Closed</span>
                    <span className="text-xs text-gray-500">
                      {inspection.status.completion || "PENDING"}
                    </span>
                  </div>
                </div>
              </CardContent>
          ...
          {inspection.defects && (
            <Card>
              <CardContent className="p-4">
                <h3 className="text-lg font-semibold mb-3 flex items-center">
                  <AlertTriangle className="mr-2 h-5 w-5 text-red-500" /> Defect Items (
                  {inspection.faultCount})
                </h3>
                  ...
                  <div className="mb-4">
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      {inspection.defects.repair.map((item, index) => (
                        <li key={`repair-${index}`} className="text-gray-700">
                          {item}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    ...
                    <ul className="list-disc pl-5 space-y-1">
                      {inspection.defects.replace.map((item, index) => (
                        <li key={`replace-${index}`} className="text-gray-700">
                          {item}
                        </li>
                      ))}
                    </ul>
          ...
          <div className="mt-6 flex justify-end gap-2">
            ...
            <Button onClick={onClose}>View Work Order</Button>
          </div>
        </div>
      </div>
    </div>
