...
const TyreSelectionForm: React.FC<TyreSelectionFormProps> = ({
  ...
  const { options: brandOptions, loading: loadingBrands } = useTyreBrandOptions();
  const { options: sizeOptions, loading: loadingSizes } = useTyreSizeOptions();
  const { options: patternOptions, loading: loadingPatterns } = useTyrePatternOptions(
    formData.brand,
    formData.size
  );
