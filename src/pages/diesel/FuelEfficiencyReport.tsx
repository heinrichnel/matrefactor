...
const FuelEfficiencyReport: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      title: 'Efficiency',
      key: 'efficiency',
      render: (_: any, record: EfficiencyData) => getEfficiencyProgress(record.efficiency, record.target),
      ...
      render: (_: any, record: EfficiencyData) => {
        ...
        return (
          <Text type={isGood ? 'success' : 'danger'}>
            {isGood ? '-' : '+'}{Math.abs(diff).toFixed(1)} L/100km
          </Text>
        );
      },
    ...
    },
    {
      ...
      onFilter: (value: string, record: EfficiencyData) => record.performance === value,
    },
    {
      title: 'Cost ($)',
      dataIndex: 'cost',
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            ...
            <Text type="secondary">
              Analyze fuel consumption and efficiency across your fleet
            </Text>
          </div>
          <Space>
        ...
        <Table 
          ...
          summary={(pageData) => {
            ...
            const pageTotalDistance = pageData.reduce((sum, item) => sum + item.distanceTraveled, 0);
            ...
            return (
              <>
                <Table.Summary.Row>
                  <Table.Summary.Cell index={0} colSpan={2}><Text strong>Summary:</Text></Table.Summary.Cell>
                  <Table.Summary.Cell index={2}><Text strong>{pageTotalDistance.toLocaleString()} km</Text></Table.Summary.Cell>
                  <Table.Summary.Cell index={3}><Text strong>{pageTotalFuel.toLocaleString()} L</Text></Table.Summary.Cell>
                  <Table.Summary.Cell index={4}><Text strong>{pageAvgEfficiency.toFixed(1)} L/100km</Text></Table.Summary.Cell>
                  <Table.Summary.Cell index={5}></Table.Summary.Cell>
                  <Table.Summary.Cell index={6}></Table.Summary.Cell>
                  <Table.Summary.Cell index={7}><Text strong>${pageTotalCost.toFixed(2)}</Text></Table.Summary.Cell>
                </Table.Summary.Row>
              </>
            );
          }}
              ...
              <Card title="Best Performer" type="inner">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Text strong style={{ color: '#3f8600' }}>7.8 L/100km</Text>
                  </div>
                </Space>
              </Card>
            </Col>
            <Col xs={24} md={8}>
              <Card title="Needs Improvement" type="inner">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text>Efficiency:</Text>
                    <Text strong style={{ color: '#cf1322' }}>9.6 L/100km</Text>
                  </div>
                </Space>
              </Card>
            </Col>
              ...
              <Card title="Potential Savings" type="inner">
                <Space direction="vertical" style={{ width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <Text strong style={{ color: '#3f8600' }}>~320 L</Text>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Text>Cost saved:</Text>
                    <Text strong style={{ color: '#3f8600' }}>~$400</Text>
                  </div>
                </Space>
              </Card>
            </Col>
