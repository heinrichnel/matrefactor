...
const ActiveTripsManager: React.FC<ActiveTripsManagerProps> = ({ onViewTrip }) => {
  ...
  const filteredTrips = useMemo(() => {
    return activeTrips.filter(t => {
      ...
      if (filterDriver && !t.driverName?.toLowerCase().includes(filterDriver.toLowerCase())) return false;
      if (filterClient && !t.clientName?.toLowerCase().includes(filterClient.toLowerCase())) return false;
      return true;
    });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowFilters(s => !s)}>{showFilters ? 'Hide Filters' : 'Show Filters'}</Button>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <select className="mt-1 w-full border rounded px-2 py-1" value={filterFleet} onChange={e => setFilterFleet(e.target.value)}>
              ...
              <div>
                ...
                <select className="mt-1 w-full border rounded px-2 py-1" value={filterDriver} onChange={e => setFilterDriver(e.target.value)}>
                  <option value="">All</option>
              ...
              <div>
                ...
                <select className="mt-1 w-full border rounded px-2 py-1" value={filterClient} onChange={e => setFilterClient(e.target.value)}>
                  <option value="">All</option>
      ...
      {Object.keys(tripsByDate).length === 0 && (
        <Card><CardContent>No active trips found.</CardContent></Card>
      )}
        ...
        <Card key={date}>
          ...
          <CardContent className="space-y-4">
            {dayTrips.map(trip => {
              ...
              return (
                <div key={trip.id} className="border rounded p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="space-y-1">
                    <div className="font-medium">{trip.fleetNumber} • {trip.route}</div>
                    <div className="text-sm text-gray-600">Driver: {trip.driverName || '—'} | Client: {trip.clientName || '—'}</div>
                    <div className="text-sm text-gray-500">Revenue: {formatCurrency(trip.baseRevenue || 0, trip.revenueCurrency || 'ZAR')} • Costs: {formatCurrency(totalCosts, trip.revenueCurrency || 'ZAR')}</div>
                    {flags > 0 && <div className="text-xs text-red-600">{flags} flagged cost(s)</div>}
                  </div>
                  <div className="flex gap-2 flex-wrap">
                    <Button variant="outline" onClick={() => onViewTrip?.(trip.id)}>View</Button>
                    {trip.status === 'active' && <Button variant="outline" onClick={() => { setStatusUpdateTrip(trip); setStatusUpdateType('shipped'); }}>Mark Shipped</Button>}
                    {(trip.status === 'active' || trip.status === 'shipped') && <Button variant="outline" onClick={() => { setStatusUpdateTrip(trip); setStatusUpdateType('delivered'); }}>Mark Delivered</Button>}
                  </div>
                </div>
              );
            })}
